...
export async function POST(request) {
  ...
  try {
    ...
    if (idempotencyKey.length < 8 || idempotencyKey.length > 100) throw new Error('Idempotency key không hợp lệ');
    const db = getSupabaseAdminClient();
    const { data, error } = await db.rpc('record_supplier_payment', { p_batch_id: batchId, p_amount_rmb: amount, p_exchange_rate: rate, p_method: method, p_reference: String(body.reference || '').trim().slice(0, 200), p_date: date, p_notes: String(body.notes || '').trim().slice(0, 2000), p_actor: auth.profile.name, p_idempotency_key: idempotencyKey });
