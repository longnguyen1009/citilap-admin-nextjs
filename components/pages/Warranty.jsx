    ...
    <section className="page-section">
      ...
      <div className="card glass p-0" style={{ overflowX: 'auto' }}>
        <table className="data-table data-table-wide" style={{ width: '100%' }}><thead><tr><th>Phiếu</th><th>Máy / Serial</th><th>Khách & Đơn</th><th>Lỗi khách báo</th><th>Tiếp nhận</th><th>Trạng thái</th><th>Chi phí (tr)</th><th /></tr></thead>
          <tbody>{rows.length === 0 ? <tr><td colSpan={8} className="empty-cell">Chưa có phiếu bảo hành nào.</td></tr> : rows.map(item => {
            const laptop = laptops.find(machine => machine.id === item.laptopId);
            return <tr key={item.id}><td style={{ color: 'var(--primary)', fontWeight: 700 }}>{item.id}</td><td><strong>{laptop?.id || item.laptopId}</strong><br /><span style={{ fontSize: '0.8rem' }}>{laptop?.name || 'Máy đã bị xóa'}</span><br /><span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>{laptop?.serial || '-'}</span></td><td>{item.customerInfo || '-'}<br /><span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{item.orderId ? `Đơn #${item.orderId}` : 'Chưa liên kết đơn'}</span></td><td>{item.issueDescription}</td><td>{item.receivedDate}</td><td><span className="status-badge status-back-tq">{item.status}</span></td><td>{Number(item.repairCost || 0).toFixed(2)}</td><td><button className="btn btn-sm btn-primary" onClick={() => openEdit(item)}>Cập nhật</button></td></tr>;
          })}</tbody>
        </table>
      </div>
      ...
      {isModalOpen && <div className="modal-backdrop active"><div className="modal-box glass" style={{ maxWidth: '760px' }}>
        ...
        <form onSubmit={handleSubmit}><div className="modal-body" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px' }}>
          ...
          <div className="form-group" style={{ gridColumn: 'span 2' }}><label>Lỗi khách báo *</label><textarea className="form-control" rows={3} value={formData.issueDescription} onChange={event => setFormData({ ...formData, issueDescription: event.target.value })} required /></div>
          <div className="form-group"><label>Trạng thái xử lý</label><select className="form-control" value={formData.status} onChange={event => setFormData({ ...formData, status: event.target.value })}>{WARRANTY_CASE_STATUS_OPTIONS.map(status => (<option key={status.key} value={status.key}>{status.label}</option>))}</select></div>
