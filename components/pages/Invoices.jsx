...
export default function Invoices({ id, query = '' }) {
  ...
  if (!id) {
    const filtered = rows.filter(row => [code(row.id),row.order_id,row.snapshot?.customer?.name,row.snapshot?.customer?.phone].join(' ').toLowerCase().includes(search.toLowerCase()));
    return <section className="invoice-workspace"><header className="invoice-toolbar"><div><p className="invoice-eyebrow">CITILAP · CHỨNG TỪ BÁN HÀNG</p><h1><FileText size={24} /> Quản lý hóa đơn</h1><p>{rows.length} hóa đơn đã xuất · Bản lưu tại thời điểm xuất</p></div><Link href="/orders" className="btn btn-primary">Đến đơn hàng</Link></header>
      ...
      <div className="invoice-paper invoice-table-scroll"><table className="invoice-table"><thead><tr><th>Hóa đơn</th><th>Khách hàng</th><th>Chi nhánh</th><th>Ngày tạo</th><th>Tổng tiền</th><th>Đã thu</th><th /></tr></thead><tbody>{filtered.map(row => { const snapshot=row.snapshot || {}; const total=Number(snapshot.total || 0); const paid=Number(snapshot.paid || 0); return <tr key={row.id}><td><strong>{code(row.id)}</strong><small>Đơn #{row.order_id}</small></td><td>{snapshot.customer?.name || '—'}<small>{snapshot.customer?.phone || '—'}</small></td><td>{snapshot.branch?.name || '—'}</td><td>{date(row.created_at)}</td><td>{invoiceMoney(total)}</td><td>{invoiceMoney(paid)}<small>{paid >= total && total > 0 ? 'Đã thanh toán đủ' : `Còn ${invoiceMoney(Math.max(total-paid,0))}`}</small></td><td><Link href={`/invoices/${row.id}`}>Xem chi tiết hóa đơn →</Link></td></tr>; })}</tbody></table>{!filtered.length && <p className="invoice-empty">Chưa có hóa đơn phù hợp. Có thể xuất khi đơn đang giao hoặc hoàn thành và đã nhận cọc.</p>}</div>
    </section>;
  }
  ...
  return <article className="invoice-workspace invoice-detail">
    ...
    <section><h2>Thanh toán & tài chính</h2><dl className="invoice-facts"><div><dt>Tổng tiền hàng</dt><dd>{invoiceMoney(total)}</dd></div><div className="invoice-total"><dt>Tổng cần thu</dt><dd>{invoiceMoney(total)}</dd></div><div><dt>Đã thu</dt><dd className="invoice-paid">{invoiceMoney(paid)}</dd></div><div><dt>Còn phải thu</dt><dd>{invoiceMoney(Math.max(total-paid,0))}</dd></div></dl><progress max="100" value={percent} aria-label="Tỷ lệ đã thanh toán" /><div className="invoice-progress-caption"><span>{percent}% đã thanh toán</span><strong>{percent >= 100 ? 'Đã TT đủ' : 'Còn công nợ'}</strong></div>
