...
export default function Procurement({ mode }) {
  ...
  return <main className="procurement-page">
    ...
    {loading ? <div className="procurement-empty">Đang tải dữ liệu…</div> : isSuppliers ? <>
      <div className="procurement-actions"><div className="procurement-search"><Search size={16}/><input placeholder="Tìm mã, tên, WeChat…" onChange={e=>{ const q=e.target.value.toLowerCase(); document.querySelectorAll('[data-supplier]').forEach(el=>el.hidden=!el.dataset.supplier.includes(q)); }}/></div><button className="btn btn-primary" onClick={()=>setSupplierDraft({...emptySupplier})}><Plus size={16}/>Thêm nhà cung cấp</button></div>
      ...
      <div className="procurement-table-wrap"><table className="procurement-table"><thead><tr><th>Mã</th><th>Nhà cung cấp</th><th>Liên hệ</th><th>Khu vực</th><th>Điểm nhận</th><th>Trạng thái</th><th/></tr></thead><tbody>{suppliers.map(s=><tr key={s.id} data-supplier={`${s.code} ${s.name} ${s.wechat_name} ${s.wechat_id}`.toLowerCase()}><td><strong>{s.code}</strong></td><td>{s.name}<small>{s.display_name}</small></td><td>{s.wechat_name||s.phone||'—'}<small>{s.wechat_id}</small></td><td>{[s.city,s.province,s.country].filter(Boolean).join(', ')}</td><td>{DESTINATIONS[s.preferred_shipping_destination]}</td><td><span className={`procurement-status ${s.active?'ok':'muted'}`}>{s.active?'Đang dùng':'Ngừng dùng'}</span></td><td><button className="btn btn-sm btn-outline" onClick={()=>setSupplierDraft({...s})}>Sửa</button></td></tr>)}</tbody></table>{!suppliers.length&&<div className="procurement-empty">Chưa có nhà cung cấp.</div>}</div>
    </> : isPayments ? <div className="procurement-table-wrap"><table className="procurement-table"><thead><tr><th>Ngày</th><th>Lô mua</th><th>Nhà cung cấp</th><th>Phương thức</th><th>RMB</th><th>Quy đổi VND</th><th>Tham chiếu</th></tr></thead><tbody>{payments.map(p=><tr key={p.id}><td>{p.payment_date}</td><td><Link href={`/purchases?id=${p.purchase_batch_id}`}>{p.purchase_batches?.batch_code}</Link></td><td>{p.suppliers?.name}</td><td>{PAYMENT_METHODS[p.payment_method]}</td><td><strong>{formatRmb(p.amount_rmb)}</strong></td><td>{formatVnd(p.amount_vnd)}</td><td>{p.reference||'—'}<small>{p.recorded_by}</small></td></tr>)}</tbody></table>{!payments.length&&<div className="procurement-empty">Chưa có thanh toán nhà cung cấp.</div>}</div> : batchDraft ?
      ...
      : detail ? <PurchaseDetail detail={detail} onBack={()=>router.push('/purchases')} onEdit={editDraft} onTransition={transition} paymentDraft={paymentDraft} setPaymentDraft={setPaymentDraft} onPay={pay}/>
      : <><div className="procurement-actions"><div/><button className="btn btn-primary" onClick={()=>openBatch()}><Plus size={16}/>Tạo lô mua</button></div><div className="procurement-table-wrap"><table className="procurement-table"><thead><tr><th>Mã lô</th><th>Ngày mua</th><th>Nhà cung cấp</th><th>Sản phẩm</th><th>Tổng tiền</th><th>Đã trả / Còn nợ</th><th>Trạng thái</th><th/></tr></thead><tbody>{batches.map(b=><tr key={b.id}><td><strong>{b.batch_code}</strong></td><td>{b.purchase_date}</td><td>{b.supplier_code}<small>{b.supplier_name}</small></td><td>{b.item_count}</td><td>{formatRmb(b.total_rmb)}</td><td>{formatRmb(b.paid_rmb)}<small className="debt">Nợ {formatRmb(b.debt_rmb)}</small></td><td><span className={`procurement-status status-${b.status.toLowerCase()}`}>{PURCHASE_STATUSES[b.status]}</span></td><td><button className="icon-link" onClick={()=>openBatch(b)}>Chi tiết <ChevronRight size={15}/></button></td></tr>)}</tbody></table>{!batches.length&&<div className="procurement-empty">Chưa có lô mua hàng.</div>}</div></>}
  </main>;
}

function PurchaseDetail({ detail,onBack,onEdit,onTransition,paymentDraft,setPaymentDraft,onPay }) {
