...
export const fetchLaptopsFromCloud = async (opts = {}) => {
  ...
  try {
    ...
    if (monthRange && !opts.all) {
      // Keep sold laptops in the result so historical orders can still resolve
      // their product after the month filter is applied.
      query = query.or(`and(import_date.gte.${monthRange.start},import_date.lt.${monthRange.next}),status.in.(repairing,available,deposited,sold,not_imported,returned_cn,skipped)`);
    }
