...
export const saveLaptopToCloud = async (laptop) => {
  ...
  try {
    ...
    assertNonNegative(dbRow, ['price_rmb', 'shipping_rmb', 'exchange_rate', 'import_price_vnd', 'battery_health', 'cycle_count']);
...
export const fetchOrdersFromCloud = async (opts = {}) => {
  ...
  try {
    ...
    const monthRange = getMonthRange(opts.monthKey);
    if (monthRange) query = query.gte('created_date', monthRange.start).lt('created_date', monthRange.next);
    const res = await query.eq('is_active', true);
    if (res.error || !res.data) return null;
    return keysToCamel(res.data);
  } catch (err) { return null; }
...
export const saveOrderToCloud = async (order) => {
  ...
  try {
    ...
    assertNonNegative(dbRow, ['sale_price', 'deposit_amount', 'cod_amount', 'amount_paid', 'debt_amount', 'credit_card_fee', 'profit_vnd']);
