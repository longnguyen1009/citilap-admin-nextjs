...
export const fetchLaptopsFromCloud = async (opts = {}) => {
  ...
  try {
    ...
    const offset = paginated ? Math.max(Number.isInteger(Number(opts.offset)) ? Number(opts.offset) : 0, 0) : 0;
    let query = paginated
      ? client.from('laptops').select('*', { count: 'exact' })
      : client.from('laptops').select('*');
    ...
    return paginated
      ? { data, total: res.count || 0, offset, limit, hasMore: offset + data.length < (res.count || 0) }
      : data;
...
export const saveOrderToCloud = async (order, recordedBy = null) => {
  ...
  try {
    ...
    await Promise.all([
      assertReferenceExists(client, 'laptops', dbRow, 'laptop_id'),
      assertReferenceExists(client, 'laptops', dbRow, 'requested_laptop_id'),
      assertReferenceExists(client, 'laptops', dbRow, 'trade_in_laptop_id'),
      assertReferenceExists(client, 'customers', dbRow, 'customer_id'),
    ]);
...
export const createOrderWithInventoryToCloud = async (order, recordedBy) => {
  ...
  if (isPresent(dbRow.laptop_id)) {
    ...
    referenceChecks.push(assertReferenceExists(client, 'laptops', dbRow, 'laptop_id'));
  }
  if (isPresent(dbRow.requested_laptop_id)) {
    assertIntegerId(dbRow, 'requested_laptop_id');
    referenceChecks.push(assertReferenceExists(client, 'laptops', dbRow, 'requested_laptop_id'));
  }
  if (isPresent(dbRow.customer_id)) {
    assertIntegerId(dbRow, 'customer_id');
    referenceChecks.push(assertReferenceExists(client, 'customers', dbRow, 'customer_id'));
  }
  if (isPresent(dbRow.trade_in_laptop_id)) {
    assertIntegerId(dbRow, 'trade_in_laptop_id');
    referenceChecks.push(assertReferenceExists(client, 'laptops', dbRow, 'trade_in_laptop_id'));
  }
...
export const rollForwardMonth = async (newMonthKey) => {
  ...
  try {
    ...
    const [{ data: openOrders, error: orderFetchError }, { data: activeLaptops, error: laptopFetchError }] = await Promise.all([
      client
        .from('orders')
    ...
    const [laptopResult, orderResult] = await Promise.all([
      laptopIds.length > 0
        ? client.from('laptops').update({ month_key: newMonthKey, updated_at: updatedAt }).in('id', laptopIds).select('id')
        : Promise.resolve({ data: [], error: null }),
      orderIds.length > 0
        ? client.from('orders').update({ month_key: newMonthKey, updated_at: updatedAt }).in('id', orderIds).select('id')
        : Promise.resolve({ data: [], error: null }),
    ]);
    if (laptopResult.error) throw laptopResult.error;
    if (orderResult.error) throw orderResult.error;
    laptopData = laptopResult.data || [];
