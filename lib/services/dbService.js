...
export const fetchLaptopsFromCloud = async (opts = {}) => {
  ...
  try {
    ...
    const monthRange = getMonthRange(opts.monthKey);
    if (monthRange && !opts.all) {
      query = query.or(`and(import_date.gte.${monthRange.start},import_date.lt.${monthRange.next}),status.in.(repairing,available,deposited,not_imported,returned_cn,skipped)`);
    }
...
export const saveLaptopToCloud = async (laptop, options = {}) => {
  ...
  try {
    ...
    if (options.create) delete dbRow.id;
    LAPTOP_PRICE_FIELDS.forEach(field => {
      if (isPresent(dbRow[field])) dbRow[field] = Number(dbRow[field]);
    });
    assertNonNegative(dbRow, ['price_rmb', 'shipping_rmb', 'exchange_rate', 'import_price_vnd', ...LAPTOP_PRICE_FIELDS, 'battery_health', 'cycle_count']);
...
export const fetchOrdersFromCloud = async (opts = {}) => {
  ...
  try {
    ...
    const monthRange = getMonthRange(opts.monthKey);
    if (monthRange && !opts.all) query = query.gte('created_date', monthRange.start).lt('created_date', monthRange.next);
    const res = await query.eq('is_active', true);
    if (res.error || !res.data) return null;
    return keysToCamel(res.data);
  } catch (err) { return null; }
...
export const saveOrderToCloud = async (order, recordedBy = null) => {
  ...
  try {
    ...
    const { data, error } = await client.rpc('update_order_with_inventory', {
      p_order: dbRow,
