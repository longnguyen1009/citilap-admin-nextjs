...
export const buildOptions = (groupKey, dbOptions = []) => {
  ...
  if (groupRows.length > 0) {
    // If DB rows exist for this group, use them exclusively
    return groupRows.sort((a, b) => a.sort_order - b.sort_order).map(o => ({ key: o.id, label: o.label }));
  }
...
export const labelToKey = (groupKey, labelValue, dbOptions = []) => {
  ...
  const byLabel = options.find(o => o.label.toLowerCase() === String(labelValue).toLowerCase());
...
const readCloudPresets = async () => {
  ...
  try {
    ...
    const { data, error } = await client.from('app_settings').select('value').eq('key', PRESET_SETTINGS_KEY).single();
