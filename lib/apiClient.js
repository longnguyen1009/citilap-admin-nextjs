...
const LAPTOP_COLS = 'id,sku,serial,name,location,category,conditionNote,chargerStatus,seller,status,priceRmb,shippingRmb,exchangeRate,importPriceVnd,wholesalePriceVnd,retailPriceVnd,trackingCode,importDate,warehouseDate,warrantySupplier,is_active,batteryHealth,isLocked,screenStatus,cameraMicStatus,mainboardStatus,created_at,partsHistory';
// Alias cho select trong Supabase query (snake_case)
const LAPTOP_SELECT = 'id,sku,serial,name,location,category,condition_note,charger_status,seller,status,price_rmb,shipping_rmb,exchange_rate,import_price_vnd,wholesale_price_vnd,retail_price_vnd,tracking_code,import_date,warehouse_date,warranty_supplier,is_active,battery_health,is_locked,screen_status,camera_mic_status,mainboard_status,created_at,parts_history';

const ORDER_SELECT = 'id,created_date,sale_online,sale_offline,note,order_type,order_status,payment_status,payment_method,delivery_status,shipping_method,laptop_id,requested_laptop_id,sale_price,deposit_amount,deposit_note,cod_amount,amount_paid,debt_amount,credit_card_fee,profit_vnd,cost_snapshot_vnd,gross_profit_snapshot_vnd,direct_cost_snapshot_vnd,net_contribution_snapshot_vnd,cost_snapshot_status,cost_snapshot_reasons,cost_snapshotted_at,trade_in_laptop_id,customer_id,customer_info,customer_address,tracking_code,ship_date,setup_note,warranty,gifts,branch_id,gift_preset,gift_accessory_ids,laptop_locked,reservation_expires_at,cancel_reason,cancelled_at,returned_at,return_reason,is_active,created_at,updated_at';
...
export const mapOrderFromDB = (dbRow) => ({
  ...
  giftPreset: dbRow.gift_preset || '',
  giftAccessoryIds: Array.isArray(dbRow.gift_accessory_ids) ? dbRow.gift_accessory_ids : [],
  ...
  returnedAt: dbRow.returned_at || null,
  returnReason: dbRow.return_reason || '',
  isActive: dbRow.is_active !== false,
  created_at: dbRow.created_at || null,
...
export const mapOrderToDB = (order) => ({
  ...
  gift_preset: order.giftPreset || '',
  gift_accessory_ids: Array.isArray(order.giftAccessoryIds) ? order.giftAccessoryIds : [],
